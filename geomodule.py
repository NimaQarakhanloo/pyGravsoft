#!/usr/bin/python
# $Id: geomodule.py 286 2009-07-26 15:36:32Z tjansson $
...
def runprogram(Icommand, Ijobfile, Ioutput):
    ...
    if os.name == "posix":
        ...
        #outputfile = open(Ioutput, 'w')
        #process = subprocess.Popen([os.path.join(pythonpath, "bin", Icommand + '.exe')],
        #stdin=open(Ijobfile, "rb"),shell=True,
        #stdout=subprocess.PIPE)

        #print "#### Program running... #####"
        #while process.poll() is None: #Checks if child process has terminated.
            #output = process.stdout.readline()
            #outputfile.writelines(output)
            #print output,
        #outputfile.close
        ...
        # subprocess is only support on python 2.4.4 and higher
        # Most Windows installations are 2.5.2. IKOS is running RHEL 4 with
        # 2.3.4. Hence the process is only added to Windows.
        # http://blog.kagesenshi.org/2008/02/teeing-python-subprocesspopen-output.html

        # So the output of the run command is sendt to the pipe object and this loop
        # picks it up and writes it to a file as well as to the screen.
        # http://ivory.idyll.org/blog/feb-07/problems-with-subprocess
        # http://code-bear.com/bearlog/2006/03/02/

        # Regarding print with extra newlines
        # http://mail.python.org/pipermail/python-dev/2001-June/015362.html
...
class statusbar:
	...
	self.statusbar = Label(Iframe, bd=1, relief=SUNKEN, anchor=W,
	width=maxwidth, text="")
